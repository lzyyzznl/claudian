...
describe('OmpConversationHistoryService', () => {
  it('hydrates from providerState sessionFile', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-'));
  ...
  it('recovers the last Omp model on the persisted active branch', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-model-history-'));
    const sessionFile = path.join(dir, 'session.jsonl');
    ...
    await expect(new OmpConversationHistoryService()
      .recoverConversationModelSelection?.(conversation, null))
      .resolves.toBe('omp:openai-codex/gpt-5.5');
  });

  it('leaves model recovery unresolved when the persisted leaf is missing', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-missing-model-leaf-'));
    const sessionFile = path.join(dir, 'session.jsonl');
    await fs.writeFile(sessionFile, [
      JSON.stringify({
        id: 'model-1',
  ...
  it('rejects an out-of-root metadata path and re-resolves by logical session id', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-home-'));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-outside-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
  ...
  it('hydrates trusted file-only Omp sessions without a logical session id', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-file-only-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
    await fs.mkdir(trustedDir, { recursive: true });
  ...
  it('hydrates detached previous sessions without making them active resume state', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-detached-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
    await fs.mkdir(trustedDir, { recursive: true });
  ...
  it('hydrates previous and active Omp session segments in chronological order', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-segments-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
    await fs.mkdir(trustedDir, { recursive: true });
  ...
  it('preserves unrelated id-less messages from different session segments', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-idless-segments-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
    await fs.mkdir(trustedDir, { recursive: true });
    const previousFile = path.join(trustedDir, 'previous.jsonl');
    const activeFile = path.join(trustedDir, 'active.jsonl');
  ...
  it('builds a fork from the segment that contains the selected checkpoint', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-segment-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
    await fs.mkdir(trustedDir, { recursive: true });
    const previousFile = path.join(trustedDir, 'previous.jsonl');
    const activeFile = path.join(trustedDir, 'active.jsonl');
  ...
  it('does not probe an archived fork source that fails path identity validation', async () => {
    const vault = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-vault-'));
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-safe-home-'));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-unsafe-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
    await fs.mkdir(trustedDir, { recursive: true });
    const activeFile = path.join(trustedDir, 'active.jsonl');
  ...
  it('hydrates an archived transcript after the configured session root changes', async () => {
    const vault = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-archive-vault-'));
    const oldRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-archive-old-'));
    const newRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-archive-new-'));
    const safeHome = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-archive-home-'));
  ...
  it('does not replace an archived path with a mismatched logical-id candidate', async () => {
    const vault = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-identity-vault-'));
    const oldRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-identity-old-'));
    const newRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-identity-new-'));
    const safeHome = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-identity-home-'));
  ...
  it('accepts a metadata path under the explicitly configured session directory', async () => {
    const configuredDir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-configured-'));
  ...
  it('does not trust a vault-local session root that is a symlink outside the vault', async () => {
    ...
    const vault = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-vault-'));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-symlink-target-'));
    const safeHome = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-safe-home-'));
    const agentDir = path.join(vault, '.omp', 'agent');
  ...
  it('builds pending fork state from source session metadata', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-state-'));
    const sessionFile = path.join(dir, 'session.jsonl');
  ...
  it('resolves file-only Omp sessions as fork sources', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-file-fork-'));
    const sessionFile = path.join(dir, 'session.jsonl');
    await fs.writeFile(sessionFile, [
  ...
  it('hydrates pending forks from the source session truncated at the checkpoint', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-'));
  ...
  it('replaces an untrusted pending-fork path with the resolved local source', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-home-'));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-outside-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
    await fs.mkdir(trustedDir, { recursive: true });
  ...
  it('sanitizes a pending-fork path even when messages are already hydrated', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-loaded-home-'));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-loaded-outside-'));
    const trustedDir = path.join(home, '.omp', 'agent', 'sessions');
    await fs.mkdir(trustedDir, { recursive: true });
    const trustedFile = path.join(trustedDir, 'source-session.jsonl');
    const outsideFile = path.join(outside, 'source-session.jsonl');
  ...
  it('does not hydrate pending forks when the checkpoint is missing', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-history-fork-missing-'));
    const sessionFile = path.join(dir, 'source.jsonl');
    await fs.writeFile(sessionFile, [
      JSON.stringify({ type: 'session', id: 'source-session' }),
      JSON.stringify({ id: 'u1', type: 'message', message: { role: 'user', content: 'First' } }),
  ...
  describe('resolveMissingConversationSession', () => {
    ...
    it('does not restore a stale path after reset and persistence reconstruction', async () => {
      ...
      expect(
        new OmpConversationHistoryService().resolveSessionIdForConversation(
    ...
    it('clears an exact stale logical binding while preserving unknown state and native files', async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-missing-reset-'));
