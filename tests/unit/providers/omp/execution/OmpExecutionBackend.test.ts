...
describe('OmpExecutionBackend', () => {
  ...
  it('keeps one execution open across native Omp retries and commits the recovered answer', async () => {
    const harness = createHarness();
    const run = harness.session.execute(createRequest());
    const eventsPromise = collect(run.events);
    let settled = false;
    ...
    expect(events.filter(event => event.type === 'notice')).toEqual([
      expect.objectContaining({ message: 'OMP is retrying the turn.' }),
      expect.objectContaining({ message: 'OMP is retrying the turn.' }),
      expect.objectContaining({ message: 'OMP retry finished.' }),
    ]);
  ...
  it('surfaces a native Omp terminal error after a non-retrying agent end', async () => {
    const harness = createHarness();
    const run = harness.session.execute(createRequest());
    const eventsPromise = collect(run.events);
    await flush();
  ...
  it('preserves a pending native Omp error when the process exits before agent end', async () => {
    const harness = createHarness();
    const run = harness.session.execute(createRequest());
    const eventsPromise = collect(run.events);
    await flush();
    ...
    kernel.close(new Error('Omp process exited'));
  ...
  it('can cancel while native Omp is waiting to retry and fences later events', async () => {
    const harness = createHarness();
    const run = harness.session.execute(createRequest());
    const eventsPromise = collect(run.events);
    let settled = false;
  ...
  it('fails before dispatch when Omp reports a replacement for the requested session', async () => {
    const sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-resume-mismatch-'));
  ...
  it('rejects steer while a relaunched kernel is awaiting resume validation', async () => {
    const sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-resume-steer-'));
    const requestedSessionFile = path.join(sessionDir, 'session-a.jsonl');
    const replacementSessionFile = path.join(sessionDir, 'session-b.jsonl');
    await fs.writeFile(
      requestedSessionFile,
  ...
  it('ignores stale resume validation from a replaced kernel', async () => {
    const sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), 'omp-resume-stale-'));
    const requestedSessionFile = path.join(sessionDir, 'session-a.jsonl');
    const replacementSessionFile = path.join(sessionDir, 'session-b.jsonl');
    await fs.writeFile(
      requestedSessionFile,
  ...
  it('materializes a file fork before launching Omp', async () => {
    ...
    const forkHeaderLine = (await fs.readFile(forkFile, 'utf8')).split('\n')[0];
    expect(JSON.parse(forkHeaderLine) as { parentSession?: string }).toMatchObject({
      parentSession: sourceFile,
    });
    const snapshot = harness.session.getSnapshot();
